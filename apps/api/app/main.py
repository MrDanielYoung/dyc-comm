...
async def _graph_get(access_token: str, path: str, params: dict[str, Any] | None = None) -> dict[str, Any]:
    headers = {"Authorization": f"Bearer {access_token}"}
    async with httpx.AsyncClient(timeout=20.0) as client:
...
def _annotate_folders(folders: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for folder in folders:
        annotated.append({**folder, **_classify_folder(folder)})
    return annotated
...
def _load_account_credentials(email: str) -> dict[str, Any]:
    if not _database_url():
        raise HTTPException(status_code=409, detail="Database-backed mailbox access is not configured.")
...
async def _graph_access_token_for_email(email: str) -> tuple[str, dict[str, Any]]:
    ...
    if not refresh_token:
        raise HTTPException(
            status_code=409,
            detail="Linked account is missing a refresh token. Run the Microsoft sign-in flow again.",
        )
...
async def _list_mail_folders(access_token: str, include_hidden: bool = False) -> list[dict[str, Any]]:
